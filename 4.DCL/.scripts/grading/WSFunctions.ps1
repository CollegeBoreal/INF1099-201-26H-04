...
function Get-ParticipationGrades {
    ...
    foreach ($line in $lines) {
        ...
        if ($line -match '^\|\s*\d+\s*\|') {
            ...
            $levels = @(103, 104, 105)  # fail, silver, gold
...
function New-LMSRubricFromEntry {
    ...
    # Build rubric
    $rubric = @(
        @{ criterionid = 45;  levelid = $Entry.readme;    remark = "Quantité README.md " }
        @{ criterionid = 46;  levelid = $Entry.image;     remark = "Présence répertoire images " }
        @{ criterionid = 47;  levelid = $Entry.abacus;    remark = "Présence de copies d'écran" }
    )
